...
The linear effect is expressed in the design matrix `(1, x)`. With `centerMatrix()` we can generate a rotation matrix which is used to rotate the design matrix that the linear effect cannot modelled anymore while the full design matrix still models the linear effect:
